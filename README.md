...
- белая канва: `canvas` — фон страницы, `white` — поверхность карточек и
  стекла, `pearl` — тон чередующихся секций, `mist`, `line`;
...
- первый экран стоит на фотографии (`public/media/hero-bg.jpeg`) и работает в
  ...
  полоса отключены;
- `components/HeroTooth.tsx` — накладка поверх фотографии: мягкое сияние и
  мерцающие точки по контуру зуба. Компонент повторяет `object-fit: cover`
  в JS (`ResizeObserver`), поэтому контур совпадает со снимком на любой
  ширине; координаты зуба в кадре измерены по файлу и вынесены в константу
  `TOOTH`. При замене фотографии её нужно пересчитать. Ниже `lg` накладка
  скрыта — там кроп увеличивает зуб за пределы экрана;
...
3. **Фон первого экрана** — `public/media/hero-bg.jpeg`. Исходник 735×413,
