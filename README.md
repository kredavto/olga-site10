...
- крупные заголовки (`.t-h1 / .t-h2 / .t-h3`) залиты вертикальным градиентом
  `--heading-ink`: глубокий фиолетовый `violet-deep` (#2E1065) → `violet`
  (#4C1D95) с золотой прожилкой `gold-ink` (#8A6420) по центру глифов.
  Градиент кладётся через `background-clip: text` на сам заголовок и на его
  вложенные `span` (это нужно для посимвольного `SplitText`), поэтому он
  вертикальный: все символы строки делят один line-box и шов совпадает.
  Мелкие заголовки карточек — чёрный `graphite`. Базовое правило цвета
  объявлено в слое `base`, а градиент — с `:not(.text-white)`, поэтому на
  тёмных секциях заголовки по-прежнему остаются белыми;
...
- чернила заголовков `violet-deep`, `violet`, `gold-ink`;
