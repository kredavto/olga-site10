...
- первый экран стоит на фотографии (`public/media/hero-bg.jpeg`) и работает в
  ...
  полоса отключены;
- `components/HeroTooth.tsx` — накладка поверх фотографии: зуб медленно
  вращается (оборот за 4 минуты), по его контуру еле заметно мерцают точки,
  от него идёт мягкий свет. Вращается не весь кадр: вторая копия снимка
  обрезана растушёванным диском с центром на зубе — круг не меняется при
  повороте, обе копии из одного файла, поэтому шва не видно. Компонент
  повторяет `object-fit: cover` в JS (`ResizeObserver`), поэтому контур
  совпадает со снимком на любой ширине; координаты зуба в кадре измерены по
  файлу и вынесены в константу `TOOTH` — при замене фотографии её нужно
  пересчитать. Рисуется в два прохода: `layer="photo"` под шторками, чтобы
  вращающаяся копия притемнялась наравне с неподвижным кадром, и
  `layer="marks"` поверх них. Ниже `lg` накладка скрыта — там кроп
  увеличивает зуб за пределы экрана;
